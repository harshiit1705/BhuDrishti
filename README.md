...
The production static frontend is hosted on Netlify and the FastAPI backend is hosted on Vercel.
The frontend API client uses `https://bhudrishti-api.vercel.app` for non-localhost deployments and can still be overridden with `window.BHUDRISHTI_API_BASE`.
`frontend/_redirects` also proxies `/api/*` through Netlify to the same Vercel backend.

Before presenting the deployed app, verify these endpoints directly in the browser:
- `https://bhudrishti-api.vercel.app/api/health`
- `https://bhudrishti-api.vercel.app/api/system/status`

Expected `/api/health` response contains `"status":"ok"` and `"project":"SIH-2026-IND-U04"`.

Important: this prototype uses SQLite/local file storage for local persistence. Vercel serverless storage is not a durable production database or file store. For a real multi-instance deployment, move uploaded datasets and workflow records to durable storage (for example a managed database/object store). The deployment fix here addresses API routing and the current production 404; it does not claim Vercel's ephemeral filesystem is durable.
